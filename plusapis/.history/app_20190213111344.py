...
def postmantenimientos():
    ...
    try:
        ...
        cursor = conn.cursor()
        datos = request.json
        sql = sqls.post_sql_mantenimientos.format(datos['numero_orden'],datos['numero_bastidor'],datos['tipo_comercial'],datos['codigo_tipo_comercial'],datos['ldm'],datos['ldc'],datos['matricula'],datos['kilometraje'],datos['anio_modelo'],datos['matriculacion'],datos['asesor_servicio'],fechaactual)
        cursor.execute(sql)
        cursor.close()
        conn.commit()
        conn.close()
